...
public class SteamVR_FirstPersonOverlay : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        error = OpenVR.Overlay.SetOverlayWidthInMeters(handle, 1f);
        if (error != EVROverlayError.None)
        {
        ...
        renderTexture = new RenderTexture(3000, 3000, 16, RenderTextureFormat.ARGBFloat);
        ...
        HmdMatrix34_t matrix = new SteamVR_Utils.RigidTransform(new Vector3(0, 0, .5f), Quaternion.identity).ToHmdMatrix34();
